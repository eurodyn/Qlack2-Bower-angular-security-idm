 ...
 */
angular.module('QAngularSecurityIDM', [])
	.provider('SecuritySrv', function() {
		...
		var config = {
			...
			token: {
				...
				// This is required when the Cookie is to be deleted, as without
				...
				// set the Cookie under a different path.
				cookiePaths: "/"
			},
			// When defferedGetUser is true, the service does not return the user
			// object via this.getUser. This is to facilitate login scenarios 
			// where the actual authentication process has finished, the user
			// is considered logged-in, however the third-party application still
			// needs to load other pieces of data before it can display its
			// homepage. The third-party application needs to ensure that
			// at some point during its initilisation logic sets this back to
			// false.
			defferedGetUser: false,
			allowCache: true,
			debug: false
		};
		...
		this.setDeferredGetUser = function(b) {
			config.deferredGetUser = b;
		}

		this.$get = ["$http", "$rootScope", "$state", "$q", "$timeout",
			function($http, $rootScope, $state, $q, $timeout) {
...
function SecurityService(config, $http, $rootScope, $state, $q, $timeout, $cookies) {
	...
	_unsetUser = function() {
		_user = null;
		_log("Unsetting user information.");
		_log("Removing HTTP header: ", config.token.headerName);
	...
	_getStoredToken = function() {
		...
		if (token !== null && token.indexOf("\"") === 0) {
			_log("Found an escaped cookie, will unescape it.");
			// Remove backslashes.
		...
		if (token !== null) {
			_log("Found a token on a Cookie, will reset this Cookie now.");
			_removeCookie(config.token.cookieName);
		} else {
			_log("Reading token from session storage with name: ",
						config.token.storageName);
	...
	_removeCookie = function(cookieName) {
		...
		for (i = 0; i < paths.length; i++) {
			...
			_log("Removing Cookie: " + cookieName + ", with command: " + cookieRemovalCmd);
			document.cookie = cookieRemovalCmd;
			_log("New Cookies after removal: ", document.cookie);
		}
	}
	 ...
	 * ***********************************************************************/
	// Sets up interceptors during state changes to check for security.
	this.setupInterceptors = function() {
	...
	// Initialise security by checking if security token is already available 
	// in the local storage to re-use it. Alternatively, security can be 
	// initialised by the caller passing a security token already received by
	// other means (token will still be validated).
	this.init = function(customToken) {
		_log("Initialising SecuritySrv.");
		var deferred = $q.defer();
		
		// If a custom token has been passed, always re-initialise security.
		if (customToken || !_initialised) {
			// Update user token from custom token or local storage/cookie.
			if (customToken) {
				_log("Using custom token:", customToken);
				token = customToken;
			} else {
				token = _getStoredToken();	
				_log("Using local token:", token);
			}
			
			if (token !== null) {
				if (token.ticket !== undefined && token.ticket !== null) {
					_log("Will now check if the ticket is valid.");
					$http.post(config.idm.validateTicket, {signedTicket: token.ticket})
						.success(function(success) {
							if (success.valid) {
								_log("Ticket found valid.");
								_setUser(token);
								_initialised = true;
				...
				_log("There is nothing to initialise from.")
				_initialised = true;
				deferred.resolve();	
			}
		} else {
			_log("SecuritySrv is already initialised.");
			_initialised = true;
			deferred.resolve();
		}
	...
	this.getUser = function() {
		return config.deferredGetUser ? null : _user;
	},
	...
	this.login = function (username, password) {
		...
		$http.post(config.idm.authenticate, authenticateRequest).then(
			function(successAuth) {
				_log("Call from auth returned.");
				if (successAuth.data !== undefined && successAuth.data !== null
					...
					_log("Retrieving user's generic permissions. Ticket header " +
						"name '" + config.token.headerName + "' with value: ",
						successAuth.data.signedTicket);
					$http.get(config.idm.genericPermissions, httpConfig)
						.success(function(successPerm) {
							_log("Permissions received successfully.");
							_setUser({
						...
						_log("Could not retrieve permissions.", error);
						$rootScope.$emit('SECURITYSRV_AUTH_HTTP_ERROR', error);
					});
				} else {
					_log("Auth was unsuccessful.");
					$rootScope.$emit('SECURITYSRV_AUTH_FAIL');
				}
			}, function (errorAuth) {
				_log("There was an error during authentication.", errorAuth);
	...
	// Sets the deferredGetUser config variable at runtiume. This method is,
	// usually, called when the third-party application has finished preloading
	// all user-related data and is about to display its homepage after a
	...
	// Returns the debug state of the SecuritySrv.
	...
	this.setTokenHeaderNames = function(headerName) {
		config.token.headerName = headerName;
		config.token.storageName = headerName;
		config.token.cookieName = headerName;
	}
}
